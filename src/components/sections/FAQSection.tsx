...
export function FAQSection() {
  return (
    <section className="py-32 bg-background relative border-t border-border/40">
      <div className="container mx-auto px-6 md:px-12">
        
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-12 max-w-7xl mx-auto">
          
          <div className="lg:col-span-5">
            <div className="sticky top-32">
              <h2 className="text-sm font-medium tracking-[0.1em] text-accent uppercase mb-6">FAQ</h2>
              <h3 className="text-4xl md:text-5xl font-heading font-bold mb-8 leading-tight tracking-tight text-foreground">
                Common questions.
              </h3>
            ...
            <Accordion className="w-full">
              {faqs.map((faq, idx) => (
                <AccordionItem key={idx} value={`item-${idx}`} className="border-border/40 px-0">
                  <AccordionTrigger className="text-left font-heading text-xl md:text-2xl font-bold hover:text-accent text-foreground transition-colors py-8">
                    {faq.question}
                  </AccordionTrigger>
                  <AccordionContent className="text-muted-foreground text-[16px] leading-relaxed pb-8 pr-12">
                    {faq.answer}
                  </AccordionContent>
                </AccordionItem>
              ))}
